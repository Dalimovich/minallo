...
test.describe('Chatbot shell role mode (production timing)', () => {
  ...
  test('the chatbot canvas never renders role-resolution loading/error text (the boot cover owns that wait)', async ({
    page,
  }) => {
    const app = new AppPage(page);
    await app.goto();
    ...
    for (const state of ['loading', 'error'] as const) {
      await page.evaluate((st) => {
        window._profileResolutionState = st;
      ...
      await expect(page.locator('[data-testid="chatbot-role-loading"]')).toHaveCount(0);
      // Role safety is unchanged: both role surfaces stay hidden while unresolved.
