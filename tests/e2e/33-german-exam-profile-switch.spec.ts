...
const TESTDAF = { german_test: 'TestDaF', german_level: 'TDN 4', german_exam_profile_id: 'testdaf_digital' };
...
test.describe('German exam workspace — live profile switching', () => {
  ...
  test('telc -> TestDaF -> telc rebuilds the exam UI without a reload or stale state', async ({ page }) => {
    test.setTimeout(90_000);
    await installProfileGate(page);
    ...
    // ── switch to Digitaler TestDaF — live, no reload ───────────────────────
    savedProfileId = 'testdaf_digital';
    await applyProfile(page, TESTDAF);
    await expect(overview).toContainText('Digitaler TestDaF');
    await expect(overview).toContainText('Prepare for your actual exam format');
    await expect(overview.locator('.gl-exam-module-name')).toHaveText(['Lesen', 'Hören', 'Schreiben', 'Sprechen']);
    await expect(overview.locator('[data-exam-module="reading"] .gl-exam-module-meta')).toContainText('7 parts');
    await expect(overview.locator('[data-exam-module="listening"] .gl-exam-module-meta')).toContainText('7 parts');
    await expect(overview.locator('[data-exam-module="writing"] .gl-exam-module-meta')).toContainText('2 parts');
    await expect(overview.locator('[data-exam-module="speaking"] .gl-exam-module-meta')).toContainText('7 parts');
    // Sprachbausteine does not exist for TestDaF: not disabled, not empty — absent.
    await expect(card('sprachbausteine')).toBeHidden();
    await expect(overview).not.toContainText('Sprachbausteine');
    // The page was never reloaded.
    expect(await page.evaluate(() => (window as unknown as { __staleMarker?: string }).__staleMarker)).toBe('telc');
    ...
    // TestDaF parts are not generatable yet: opening one is refused, never routed to TELC's view.
    await card('reading').click();
    await expect(page.locator('#glReadingView')).toBeHidden();
    expect(generateBodies.filter((b) => b.profileId === 'telc_c1_hochschule')).toHaveLength(0);
    ...
    // The client never asks for a specific exam: the manifest request carries no profile id.
    expect(manifestBodies.length).toBeGreaterThanOrEqual(2);
    for (const body of manifestBodies) expect(body).not.toMatch(/profileId|telc|testdaf/i);
    ...
    await expect(overview.locator('.gl-exam-module-name')).toHaveText(['Lesen', 'Hören', 'Sprachbausteine', 'Schreiben', 'Sprechen']);
    await expect(card('sprachbausteine')).toBeVisible();
    await expect(overview).not.toContainText('TestDaF');
  });

  test('Goethe -> TestDaF -> Goethe rebuilds the exam UI without a reload or stale state', async ({ page }) => {
    test.setTimeout(90_000);
    await installProfileGate(page);

    let savedProfileId = 'goethe_c1';
    const manifestBodies: string[] = [];
    const generateBodies: Record<string, unknown>[] = [];
    await page.route('**/api/ai/german-exam/manifest', (route: Route) => {
      manifestBodies.push(route.request().postData() || '');
    ...
    // Something profile-specific to discard: a held prefetch/prepared state marker.
    await page.evaluate(() => {
      (window as unknown as { __staleMarker?: string }).__staleMarker = 'goethe';
    });
    ...
    await expect(overview).not.toContainText('Sprachbausteine');
    // The page was never reloaded.
    expect(await page.evaluate(() => (window as unknown as { __staleMarker?: string }).__staleMarker)).toBe('goethe');

    // TestDaF parts are not generatable yet: opening one is refused, never routed to Goethe's view.
    await card('reading').click();
    await expect(page.locator('#glReadingView')).toBeHidden();
    expect(generateBodies.filter((b) => b.profileId === 'goethe_c1')).toHaveLength(0);

    // The client never asks for a specific exam: the manifest request carries no profile id.
    expect(manifestBodies.length).toBeGreaterThanOrEqual(2);
    for (const body of manifestBodies) expect(body).not.toMatch(/profileId|goethe|testdaf/i);
