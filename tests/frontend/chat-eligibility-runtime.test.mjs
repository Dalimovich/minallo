...
test('retry eligibility uses saved stable IDs when the source library is unavailable', () => {
  const r = runtime();
  const chat = { selectedSourceIds: ['missing-source'], courseFileScope: 'specific_files',
    sourceMode: 'auto', courseId: 'course-a' };
  const grounding = { retrievalScope: { type: 'documents', documentIds: ['doc-a', 'doc-b'] } };
  const result = r.ragEligibility([{ role: 'user', text: 'Explain torque' }], chat, 'course-a', grounding);
  assert.ok(result);
  assert.deepEqual(Array.from(result.documentIds), ['doc-a', 'doc-b']);
  assert.equal(JSON.stringify(result.groundingRequest), JSON.stringify(grounding));
});
