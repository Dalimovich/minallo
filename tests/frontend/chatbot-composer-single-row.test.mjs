...
test('source modes live in an expandable view inside Add files', () => {
  assert.match(composer, /ncb-add-files-source-trigger/);
  assert.match(composer, /data-view="source" hidden/);
  assert.doesNotMatch(composer, /ncb-add-files-auto|ncb-add-files-selected/);
  assert.match(css, /\.ncb-add-files-view\[hidden\]\s*\{ display: none; \}/);
});
