...
def intent_style_instruction(intent: AcademicIntent | str | None) -> str:
    ...
    if intent == AcademicIntent.CASE_OR_APPLICATION_REASONING:
        ...
        lines.extend([
            ...
            # Item 4 / 7.5 — source hygiene: don't turn admin/intro slides into questions.
            "- USE ONLY TECHNICAL CONTENT. Base every Aufgabe on the subject matter of the lecture/exercise. NEVER build a question from a title page, agenda, learning-objectives, organizational/admin, contact, schedule, info-event or QR-code slide (e.g. do NOT ask to 'analyse the communication structure / layout / target groups / QR code of the info event'). If a selected file contains only such non-technical material, do NOT invent a question for it — skip it and add one short note at the top that it was skipped (no technical content). Slide layout, room numbers and QR codes are never exam material unless the student explicitly asked for a communication/design analysis.",
            ...
            "- Under each subpart give 3-5 concrete bullet points (scale with the points) using the EXACT technical terms from the source, with enough explanation that a student sees what earns the marks. For calculation subparts state the formula AND the expected/worked result; for classification subparts give the actual categories; for 'name N …' subparts list N real items; for 'explain the process' subparts give the ordered process steps.",
