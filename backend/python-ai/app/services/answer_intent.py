...
def intent_style_instruction(intent: AcademicIntent | str | None) -> str:
    ...
    if intent == AcademicIntent.CASE_OR_APPLICATION_REASONING:
        ...
        lines.extend([
            ...
            "- Produce one `## Aufgabe N: <lecture/file name> — <points> Punkte` section per selected source file, each with a `**Based on / inspired by:** [Source N] — <file name>` line directly under the heading. This is a newly generated variant, so NEVER label it simply `Source:` or imply the professor wrote the generated wording/values. Distribute the ~100 points roughly EVENLY across the files (≈ 100 / number-of-files per Aufgabe); NEVER give one Aufgabe 30-40 points while others get a handful — balanced, smaller tasks make a realistic 90-minute exam.",
