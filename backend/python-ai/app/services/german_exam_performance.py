...
def record_attempts(user_id: str, exam_family: str, exam_variant: str | None, target_level: str, items: list[AttemptItem]) -> dict:
        ...
        rows.append(
            {
                ...
                "first_attempt_correct": None if resolved["module"] in {"writing", "speaking"} else item.first_attempt_correct,
                "final_correct": None if resolved["module"] in {"writing", "speaking"} else item.final_correct,
    ...
    if rows:
        ...
        writing_rows = [row for row in rows if row["module"] in {"writing", "speaking"} and row["generation_id"]]
        other_rows = [row for row in rows if row not in writing_rows]
        sb = get_supabase()
        for row in writing_rows:
            row["id"] = str(uuid.uuid5(uuid.NAMESPACE_URL,
                f"minallo:{row['module']}:{user_id}:{row['profile_id']}:{row['generation_id']}:{row['item_id']}"))
        if writing_rows:
