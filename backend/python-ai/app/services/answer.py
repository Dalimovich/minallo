...
4e. Inclusive pass/counting check: when a tool, wheel, sensor, strip, or field of width $b$ covers a total width $B$ and advances by an effective step $s$ (after overlap), distinguish MOVES from PASSES. The first placement already covers $b$, so the number of additional moves is $m = \\lceil(B-b)/s\\rceil$ and the total number of passes/positions is $n = 1 + m$. Never report $m$ as $n$. Verify independently that the last pass reaches the far edge: $b + (n-1)s \\ge B$, while one fewer pass does not: $b + (n-2)s < B$. Example: $B=100$ mm, $b=10$ mm, $s=9$ mm gives $m=10$ moves but $n=11$ passes.
{EQUATION_READABILITY_RULE}
