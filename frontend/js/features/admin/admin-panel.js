        ...
        users.forEach((u) => {
            ...
            reindexBtn.addEventListener('click', async function () {
                ...
                try {
                    ...
                    if (!n) {
                        if (typeof window.showToast === 'function')
                            window.showToast('No documents', 'Course "' + courseId + '" has 0 docs for this user.');
                        return;
                    }
                    if (!window.confirm('Reindex ' + n + ' document(s) in course "' + courseId + '"? This clears chunks/pages and re-runs the indexer.'))
                        return;
