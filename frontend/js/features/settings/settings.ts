export interface ServerSettings {
  ...
  language?: string;
  yt_playlists?: unknown;
  [k: string]: unknown;
}

export function applySettings(raw?: ServerSettings | null): void {
  const s: ServerSettings = raw || {};
  ...
  if (typeof s.auto_open_ai === 'boolean') {
    const ao = document.getElementById('settingsAutoOpen') as HTMLInputElement | null;
    if (ao) {
  ...
  if (typeof s.save_chat_history === 'boolean') {
    const sc = document.getElementById('settingsSaveChat') as HTMLInputElement | null;
    if (sc) {
