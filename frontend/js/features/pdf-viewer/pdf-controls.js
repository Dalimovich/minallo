export function initPdfControls(options) {
    function updateZoomPct() {
        ...
        if (el)
            el.textContent = Math.round(options.getPdfScale() * 100) + '%';
    }
    function pdfVisiblePage() {
        if (!options.getPdfShowAll())
            return options.getPdfPage();
    ...
    document.getElementById('pdfAll')?.addEventListener('click', () => {
        ...
        if (btn)
            btn.textContent = options.getPdfShowAll() ? 'Single page' : 'All pages';
        options.renderPages();
    });
