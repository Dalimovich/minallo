// ── YouTube IFrame Player API shims ─────────────────────────────────────
...
let _stEndTime: number | null = null;
let _stPhase: Phase = 'focus';
let _stCycle = 0;
let _stSettings: TimerSettings = { focus: 25, shortBreak: 5, longBreak: 15, cycles: 4 };
let _stTech = 'pomodoro';
let _stMusicEnabled = true;
let _stMusicMuted = false;
let _stMusicSrc: MusicSrc = 'lofi';

const _ST_STORAGE_KEY = 'ss_focus_timer_v1';

const _stPresets: Record<string, TimerSettings | null> = {
...
let _ytPlayer: YTPlayer | null = null;
let _ytPlayerReady = false;
let _ytPendingList: string | null = null;
let _ytProgressInterval: ReturnType<typeof setInterval> | null = null;

function _stFmt(s: number): string {
  const m = Math.floor(s / 60);
  const sec = s % 60;
  return (m < 10 ? '0' : '') + m + ':' + (sec < 10 ? '0' : '') + sec;
}

function _stLockGames(lock: boolean): void {
...
function _stEnsureYTApi(cb: () => void): void {
...
function _stCreatePlayer(customList: string | null): void {
  ...
  if (progressEl) {
    ...
    progressEl.addEventListener('change', function () {
      ...
      try {
        ...
        if (dur > 0) _ytPlayer.seekTo(dur * (Number(this.value) / 100), true);
      } catch {
        /* ignore */
      }
    });
  ...
  const playerCfg: YTPlayerConfig = {
    height: '180',
    width: '320',
    ...
    onReady: function (e) {
      ...
      _ytProgressInterval = setInterval(() => {
        ...
        try {
          ...
          const range = document.getElementById('smcProgress') as HTMLInputElement | null;
          const curEl = document.getElementById('smcCurrent');
          const durEl = document.getElementById('smcDuration');
          if (range && dur > 0) range.value = String((cur / dur) * 100);
...
function _stShowMusicControls(show: boolean): void {
  const holder = document.getElementById('stYTHolder');
  if (!holder) return;
  if (show) {
    const pill = document.getElementById('stMusicPill');
...
function _stShowDonePopup(isBreakEnd: boolean): void {
...
function _stRemainingFromClock(): number {
  if (_stEndTime == null) return _stSecondsLeft;
...
function _stRestore(): void {
  ...
  if (!raw) return;
  let saved: StoredTimer;
  try {
    saved = JSON.parse(raw) as StoredTimer;
  } catch {
    return;
  }
  if (!saved || saved.v !== 1 || !saved.running) return;
...
export function openStudyPopup(): void {
...
export function closeStudyPopup(): void {
  const popup = document.getElementById('stPopup');
  const overlay = document.getElementById('stOverlay');
...
export function initStudyTimer(): void {
  ...
  document.addEventListener('click', (e) => {
    ...
    const srcCard = target.closest<HTMLElement>('.st-music-src');
    if (srcCard) {
    ...
    const card = target.closest<HTMLElement>('.st-tech-card');
    if (card) {
      ...
      if (preset) {
        ...
        if (fv) fv.textContent = String(_stSettings.focus);
        const sv = document.getElementById('stShortVal');
        if (sv) sv.textContent = String(_stSettings.shortBreak);
        const lv = document.getElementById('stLongVal');
        if (lv) lv.textContent = String(_stSettings.longBreak);
        const cv = document.getElementById('stCyclesVal');
        if (cv) cv.textContent = String(_stSettings.cycles);
      }
      return;
    }

    const stepBtn = target.closest<HTMLElement>('.st-step-btn');
    if (stepBtn) {
      const fieldRaw = stepBtn.dataset['field'];
      const dirRaw = stepBtn.dataset['dir'];
      if (!fieldRaw || !dirRaw) return;
      const field = fieldRaw as SettingsField;
      const dir = parseInt(dirRaw, 10);
      const minV: Record<SettingsField, number> = {
        focus: 1,
        shortBreak: 1,
        longBreak: 1,
        cycles: 1,
      };
      const maxV: Record<SettingsField, number> = {
        focus: 180,
        shortBreak: 60,
        longBreak: 60,
        cycles: 10,
      ...
      const ids: Record<SettingsField, string> = {
        focus: 'stFocusVal',
      ...
      if (el2) el2.textContent = String(_stSettings[field]);
      return;
    }
  ...
  document.addEventListener('visibilitychange', () => {
    ...
    if (!_stRunning || _stPaused || _stEndTime == null) return;
    _stSecondsLeft = _stRemainingFromClock();
    _stUpdateMini();
    if (_stSecondsLeft <= 0) {
      if (_stTimer) clearInterval(_stTimer);
